...
import { ensureDir, directoryFileSort, selectMigrations, MIGRATION_DIRECTION } from "../utils";
var fs = require('fs');
...
describe('Test Select Migration', () => {
    ...
    describe('When testing forward', () => {
        it('SHOULD 0000 -> 0003 ', () => {
            const { direction, selected } = selectMigrations('0000', '0003', migrations);
            expect(direction).toBe(MIGRATION_DIRECTION.FORWARD)
            expect(selected).toEqual(['0001', '0002', '0003'])
        })
        it('SHOULD 0001 -> 0003 ', () => {
            const { direction, selected } = selectMigrations('0001', '0003', migrations);
            expect(direction).toBe(MIGRATION_DIRECTION.FORWARD)
    ...
    describe('When testing reverse', () => {
        it('SHOULD 0003 -> 0001 ', () => {
            const { direction, selected } = selectMigrations('0003', '0001', migrations);
            expect(direction).toBe(MIGRATION_DIRECTION.REVERSE)
            expect(selected).toEqual(['0003', '0002'])
        })
        it('SHOULD 0001 -> 0003 ', () => {
            const { direction, selected } = selectMigrations('0002', '0001', migrations);
            expect(direction).toBe(MIGRATION_DIRECTION.REVERSE)
    ...
    describe('WHEN for current and target are same', () => {
        it('SHOULD 0003 -> 0003 ', () => {
            const { direction, selected } = selectMigrations('0003', '0003', migrations);
