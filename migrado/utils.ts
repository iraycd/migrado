...
export const directoryFileSort = async (directory: string): Promise<FilePathObject> => {
    // const files = await klaw(directory);
    return new Promise((resolve, reject) => {
        let files: any = {};
        let items: any = klawSync(directory, { nodir: true })
        if (items) {
            items.map((item: any) => {
...
export const selectMigrations = (current: string, target: string, migrations: string[]): SelectedMigrationResponse => {
    if (target > current) {
        return {
            direction: MIGRATION_DIRECTION.FORWARD,
            selected: migrations.filter((migrationPath) => (current < migrationPath && migrationPath <= target))
        }
    }
    if (target < current) {
        return {
            direction: MIGRATION_DIRECTION.REVERSE,
            selected: migrations.reverse().filter((migrationPath) => (target < migrationPath && migrationPath <= current))
        }
    }
    return {
        direction: null,
